...
void ABlackHole::OverLapInnerSphere(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OthrComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor)
	{
		OtherActor->Destroy();
...
void ABlackHole::Tick(float DeltaTime)
{
	...
	for (int32 i = 0; i < OverlappingComps.Num(); i++)
	{
		...
		{
			// the component we ae looking for! It needs to be simulating in order to aply forces.

			const float SphereRadius = OuterSphereComponent->GetScaledSphereRadius();
			const float ForceStrength = -2000; // Negative value to maek it pull towards the origin instead of pushing away
